...
${extraViews(assets.extraPhotos, model.locale)}
  ...
  <div class="alert">
    <div class="warningTitle">${t('AVOID SELF-MEDICATION')}</div>
    <div>${h(model.avoidSelfMedicationWarning)}</div>
    <div class="alertRule"></div>
    <div><b>${t('SpotOn reminder')}:</b> ${h(model.disclaimer)}</div>
...
function styles(tier: { fg: string; bg: string; border: string }): string {
  ...
  * { box-sizing: border-box; }
  ...
  .alert {
    ...
    padding: 5.5pt 9pt; font-size: 7.25pt; color: ${C.answerYes}; text-align: justify; line-height: 1.35;
  }
